...
There are multiple methods to determine protein topology based on amino acid sequence. However, based on literature and also with the old pipeline in mind, I decided to use [TOPCONS](https://doi.org/10.1093/nar/gkv485). 

First thing was to retrieve all peptide sequences from a database, went for ensembl and all ensembl_peptide_ids for this. Sequences were filtered for to only include protein coding sequences, and have to contain an ITIM sequence to already cut down on number of sequences that have to be run. The remaining sequences in the .fa fasta file have then to be loaded into TOPCONS (run rst_ekt3evqo, done on 24-06-2022). This includes in total 96,457 protein sequences.
...
if(!file.exists(paste0(processedF,"topologies_anyITIM.rds"))){
  ...
  topcons_result <- read_lines("./data/220624_topcons_results.txt",
                             skip = 4)
...
if(!dir.exists("./data/pipeline/")){
  dir.create("./data/pipeline/")
}
